...
function add(model, path, prototype) {
  function rpcClosure(model, path, method) {
    ...
    return function rpc() {
      ...
      model.call('derby-ar', data, function (err) {
        // TODO: Break current API so that one should always assume there's an err passed as firt var - or? Then we can call the cb directly
